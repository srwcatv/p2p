    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#loginInfoDisplay").typeahead({
                minLength: 3,//第几个字符开始查询
                items: 5,//下拉选中最多显示条数
                source: function (query, process) {//数据源加载
                    $.post("/vedioAuth_autocomplate.do", {keyword: query}, function (data) {
                        if (data && data.length){
                            process(data);
                        }
                    });
                ...
                <form id="searchForm" class="form-inline" method="post"
                    ...
                    <div class="form-group">
                        <label>申请时间</label> <input class="form-control"
                                                   style="width: 130px;" type="text" name="beginTime"
                                                   id="beginTime" value="${(qo.beginTime?string('yyyy-MM-dd'))!''}"/>到
